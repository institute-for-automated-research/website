...
for (const line of regText.split('\n')) {
  ...
  const pg = line.match(/^    page:\s*(\S+)/);
  if (pg && cur) regPage[cur] = pg[1];
}

// --- directory == tier invariant (issue #34) ---------------------------------
// Each access tier owns exactly one docs subdirectory so the autogenerated wiki
// sidebar (astro.config.mjs) cannot drift from the registry. Every registry
// `page:` pointer must live in the directory its tier maps to, and the file must
// exist. This is fatal: a misfiled page silently mislabels its access claim.
const TIER_DIR = {
  public: 'datasets',
  'licensed-commercial': 'commercial',
  'proprietary-confidential': 'confidential',
};
let tierDirErrors = 0;
for (const slug of Object.keys(regPage)) {
  const page = regPage[slug];
  const tier = reg[slug];
  const wantDir = TIER_DIR[tier];
  const dir = page.split('/')[0];
  if (!wantDir) {
    console.error(`DIR!=TIER: ${slug} has tier '${tier}' with no owning directory (page ${page})`);
    tierDirErrors++;
    continue;
  }
  if (dir !== wantDir) {
    console.error(`DIR!=TIER: ${slug} tier '${tier}' wants ${wantDir}/ but page is ${page}`);
    tierDirErrors++;
  }
  const abs = join(root, 'src/content/docs', `${page}.md`);
  try { readFileSync(abs); } catch {
    console.error(`MISSING PAGE FILE: ${slug} -> ${page}.md does not exist`);
    tierDirErrors++;
  }
}
if (tierDirErrors > 0) {
  console.error(`[check-dataset-access] FAIL: ${tierDirErrors} directory!=tier (or missing-file) error(s). See above.`);
  process.exit(1);
}
console.log(`[check-dataset-access] dir==tier OK (${Object.keys(regPage).length} page pointers checked)`);
